// NOVA Arsenal v10.1 — Engine Principal Corrigida
...
class NovaEngine {
  ...
  async loadProxies() {
    ...
    try {
      ...
      document.getElementById("dot-ps").className = "proxy-dot" + (d.sources && d.sources.proxyscrape > 0 ? " online" : "");
      document.getElementById("dot-gn").className = "proxy-dot" + (d.sources && d.sources.geonode > 0 ? " online" : "");
      document.getElementById("dot-pf").className = "proxy-dot" + (d.sources && d.sources.proxifly > 0 ? " online" : "");
      
      document.getElementById("label-ps").textContent = "ProxyScrape (" + (d.sources ? d.sources.proxyscrape : 0) + ")";
      document.getElementById("label-gn").textContent = "GeoNode (" + (d.sources ? d.sources.geonode : 0) + ")";
      document.getElementById("label-pf").textContent = "Proxifly (" + (d.sources ? d.sources.proxifly : 0) + ")";
  ...
  async deploy() {
    ...
    const allMethods = methods.l7.concat(methods.l4);
